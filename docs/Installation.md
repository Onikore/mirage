...
**Что делает скрипт:**
1. Клонирует (или обновляет, если уже клонировано в `/opt/mirage-src`) исходники.
2. Ставит Go 1.25+ с go.dev, если системный `go` отсутствует или старее.
3. Компилирует Mirage и копирует его в `/usr/local/bin/mirage`.
4. Генерирует криптографические ключи, складывает их в `/etc/mirage/` с правами `600` (только root).
5. Создаёт systemd-службу `mirage.service` с оптимальными настройками.
6. Запускает службу и выводит на экран готовые ключи для настройки вашего клиента.
...
### Требования:
- Установленный **Go 1.25** или новее (см. `go.mod`) -- многие дистрибутивные
  пакеты `golang` в apt/yum старее; `install.sh` при автоматической
  установке сам ставит нужную версию с go.dev, если системная слишком старая.
