...
5. **Ротация ключей и rate-limit.** Операционная обвязка вокруг уже
   настоящего Noise-рукопожатия и анти-replay кэша (см. replay.go) — сейчас
   единственный psk/server_priv живут вечно и ничем не ограничены по
   частоте попыток.

## Дисклеймер

Ещё не боевой код: нет ротации ключей, нет rate-limit, ServerHello всё ещё
не замаскирован. Само рукопожатие теперь на полноценном Noise (flynn/noise),
не самодельное, и защита от replay msg1 есть (см. replay.go) — но остальная
операционная обвязка (пп. 1-5 выше) ещё не сделана. Для реального
