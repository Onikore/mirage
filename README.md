...
анти-зонд-fallback, client->server рукопожатие замаскировано под настоящий
Chrome ClientHello (uTLS). Внешние зависимости — uTLS (байт-в-байт
совместимые TLS-фингерпринты браузеров) и flynn/noise (аудированная
реализация Noise Protocol Framework вместо самодельной крипто-последовательности).
...
5. **Ротация ключей.** Единственные psk/server_priv живут вечно — нет
   способа завести второй валидный psk на переходный период и вывести
   старый из обращения без остановки сервера. Rate-limit (per-IP
   token-bucket) уже сделан — см. ratelimit.go.

## Дисклеймер

Ещё не боевой код: нет ротации ключей, ServerHello всё ещё не замаскирован.
Само рукопожатие теперь на полноценном Noise (flynn/noise), не самодельное,
защита от replay msg1 (replay.go) и per-IP rate-limit (ratelimit.go) есть —
но остальная операционная обвязка (пп. 1-5 выше) ещё не сделана. Для
реального использования — форкать sing-box/xray и переиспользовать их
обкатанные транспорт и Reality.
