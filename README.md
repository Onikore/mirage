...
ротация psk без остановки сервера) уже сделана — см. ratelimit.go,
replay.go, pskset.go. `server_priv` при этом по-прежнему один и не
ротируется (для этого нужна была бы смена ключа при живых соединениях —
не делали, за пределами этой итерации).

## Дисклеймер

Ещё не боевой код: ServerHello всё ещё не замаскирован, QUIC/мультиплекс/
shaping не сделаны, `server_priv` не ротируется. Само рукопожатие теперь на
полноценном Noise (flynn/noise), не самодельное, есть защита от replay msg1
(replay.go), per-IP rate-limit (ratelimit.go) и ротация psk без остановки
сервера (pskset.go) — но пп. 1-4 выше ещё не сделаны. Для реального
